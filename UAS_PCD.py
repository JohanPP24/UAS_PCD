...
def apply_filter():
    ...
    filtered_img.thumbnail((800, 800))
...
# Create the top section with white background
top_canvas = tk.Canvas(canvas, width=800, height=300, bg="#FFFFFF")
top_canvas.place(x=0, y=0)

# Create the original image canvas and label
original_image_canvas = tk.Canvas(top_canvas, width=600, height=500)
...
filtered_image_canvas = tk.Canvas(top_canvas, width=600, height=500)
...
# Create the bottom section with bright blue background
bottom_canvas = tk.Canvas(canvas, width=800, height=300, bg="#000080")
...
text_label1 = tk.Label(
    canvas, text="Johan Putra Pradana 312110314", font=("Times New Roman", 12, "bold"))
text_label1.place(relx=0.5, rely=0.9, anchor="center")

text_label2 = tk.Label(
    canvas, text="UNIVERSITAS PELITA BANGSA", font=("Times New Roman", 12, "bold"))
